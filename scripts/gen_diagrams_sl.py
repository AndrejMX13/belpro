#!/usr/bin/env python3
"""Generates three architecture diagram PNGs for Belpro_Arhitektura_Sl.docx.

Output: scripts/diagrams/fig1_arhitektura.png
...
def arrow_h_dotted(draw, x1, x2, y, color=DASHED_COL, width=2):
    """Dashed horizontal arrow (single line, for Vodja connection)."""
    seg, gap, cx = 8, 6, x1
    while cx < x2 - 9:
...
def fig1():
    ...
    # Row 1 boxes
    draw_box(d, CA - BW//2, R1Y, BW, BH, TEAL_DARK, "Prostovoljec", "WhatsApp")
    draw_box(d, CB - BW//2, R1Y, BW, BH, TEAL_DARK, "Evolution API", "API Prehod")
    draw_box(d, CC - BW//2, R1Y, BW, BH, TEAL_DARK, "n8n",           "Avtomatizacija")
    draw_box(d, CD - BW//2, R1Y, BW, BH, ORANGE,    "Nadzorna pl.",  "Spl. vmesnik + API")

    # Vodja (smaller, top-right)
    MW = 100; MH = 54
    draw_box(d, CE - MW//2, R1Y + 5, MW, MH, GRAY_BOX, "Vodja", "Brskalnik / Tel.", r=8)

    # Row 2 boxes
    draw_box(d, CB - BW//2, R2Y, BW, BH, GRAY_BOX,  "Whisper AI",   "Transkripcija")
    draw_box(d, CC - BW//2, R2Y, BW, BH, TEAL_DARK, "PostgreSQL",   "Baza podatkov")
    draw_box(d, CD - BW//2, R2Y, BW, BH, GREEN_BOX, "Gmail",        "Pošiljanje e-pošte")
    ...
    # Nadzorna plošča → Vodja (dashed horizontal)
    ...
    d.text((46, ly - 7), "Tok podatkov", fill=(110, 110, 110), font=fl)
    arrow_v(d, 148, ly - 6, ly + 2, color=DASHED_COL, dashed=True)
    d.text((157, ly - 7), "S pomočjo UI", fill=(110, 110, 110), font=fl)
    arrow_h_dashed(d, 258, 286, ly, color=DOUBLE_COL)
    d.text((292, ly - 7), "Sinhronizacija", fill=(110, 110, 110), font=fl)

    return img


# ── Sl. Fig 2: Petstopenjski potek dela prostovoljca ──────────────────────────
def fig2():
    H = 150
    img = Image.new("RGB", (W, H), LIGHT_BG)
    d = ImageDraw.Draw(img)
    ...
    steps = [
        ("① Posnami",      "Glasovno sporočilo\n+ opcijska slika"),
        ("② UI obdela",    "Whisper prepiše\nnormalizira dialekt"),
        ("③ Potrdi",       "Prostovoljec pregleda\npotrdi / uredi"),
        ("④ Vodja odobri", "Vodja potrdi\nprek WhatsApp"),
        ("⑤ Končano",      "Vnos shranjen\npr. obveščen"),
    ]
...
# ── Sl. Fig 3: Mesečni cikel poročanja ────────────────────────────────────────
def fig3():
    H = 138
    img = Image.new("RGB", (W, H), LIGHT_BG)
    d = ImageDraw.Draw(img)
    ...
    steps = [
        ...
        ("PDF ustvarjen",  "Za prostovoljca\n+ zbirno"),
        ("E-pošta poslana","Prostovoljcu\n& vodji"),
        ("Oddaja CSD",     "Prostovoljec podpiše\n& odda"),
    ]
